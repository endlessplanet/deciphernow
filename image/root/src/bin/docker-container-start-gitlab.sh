...
do
    ...
    then
        docker-container-start-cloud9 --host-name ${PROJECT_NAME} &&
            ...
            docker container exec --interactive --tty $(cat ${HOME}/docker/containers/cloud9-${PROJECT_NAME}) git -C /opt/docker/workspace/${PROJECT_NAME}/repo git fetch upstream ${MASTER_BRANCH} &&
            docker container exec --interactive --tty $(cat ${HOME}/docker/containers/cloud9-${PROJECT_NAME}) git -C /opt/docker/workspace/${PROJECT_NAME}/repo git checkout upstream/${MASTER_BRANCH} &&
            docker container exec --interactive --tty $(cat ${HOME}/docker/containers/cloud9-${PROJECT_NAME}) git -C /opt/docker/workspace/${PROJECT_NAME}/repo git checkout -b scratch/$(uuidgen) &&
