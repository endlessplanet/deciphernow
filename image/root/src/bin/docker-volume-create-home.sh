...
then
    docker volume create --label expiry=$(date --date "${EXPIRY}" +%s) > ${HOME}/docker/volumes/home &&
        ...
        echo "${UPSTREAM_GITLAB_ID_RSA}" | docker container run --interactive --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 tee user/.ssh/gitlab_upstream_id_rsa &&
        echo "${ORIGIN_GITLAB_ID_RSA}" | docker container run --interactive --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 tee user/.ssh/gitlab_origin_id_rsa &&
        echo "${UPSTREAM_GITHUB_ID_RSA}" | docker container run --interactive --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 tee user/.ssh/github_upstream_id_rsa &&
        echo "${ORIGIN_GITHUB_ID_RSA}" | docker container run --interactive --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 tee user/.ssh/github_origin_id_rsa &&
        ...
        docker container run --interactive --tty --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 chmod 0600 user/.ssh/config user/.ssh/gitlab_upstream_id_rsa user/.ssh/gitlab_origin_id_rsa user/.ssh/github_upstream_id_rsa user/.ssh/github_origin_id_rsa user/.ssh/chimera_id_rsa &&
        ...
        ssh-keyscan github.com | docker container run --interactive --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 tee -a user/.ssh/known_hosts &&
        ssh-keyscan -p 2233 54.173.144.101 | docker container run --interactive --rm --mount type=volume,source=$(cat ${HOME}/docker/volumes/home),destination=/home --workdir /home alpine:3.4 tee -a user/.ssh/known_hosts &&
