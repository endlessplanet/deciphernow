...
ADVENTURE_NAME="${1}" &&
    ...
    sudo docker container create --cidfile ${SHELL_CIDFILE} --volume /var/run/docker.sock:/var/run/docker.sock:ro fedora:26 &&
...
#    sudo docker container start ${SHELL_CIDFILE} &&
    sudo docker container start ${CLOUD9_CIDFILE} &&
